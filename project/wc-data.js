...
// qualification projection engine. Plain ES module (business logic only).
//
// ── LIVE DATA HOOK ───────────────────────────────────────────────────────────
// Results are seeded best-effort as of ~June 21, 2026 (mid group stage). Some
// scores are approximate — edit DEFAULT_RESULTS or pass a fresh results object
// to project() to refresh everything. Fixture order per group is fixed (see FIX).
// ─────────────────────────────────────────────────────────────────────────────
...
export const FIX = [[0,1],[2,3],[0,2],[3,1],[3,0],[1,2]];
...
const S3 = SLOTS.filter(s=>s.b.t==='3').map(s=>({m:s.m,allow:s.b.allow}));
...
export function currentStandings(results){
  return GKEYS.map(g=>{
...
export function project(results, runs=3000){
  const tp={}; // team -> counts
  Object.keys(TEAMS).forEach(id=>tp[id]={w:0,r:0,t:0,adv:0});
  const side={}; // (m+'A'/'B') -> {team:count}
  const bump=(key,id)=>{ (side[key]||(side[key]={})); side[key][id]=(side[key][id]||0)+1; };

  for(let run=0; run<runs; run++){
    const W={},R={},TH={}; // group -> teamId
    const thirds=[]; // {g,id,pts,gd,gf}
    for(const g of GKEYS){
      const t=table(g,results,true);
      const o=t.order;
      W[g]=t.ids[o[0]]; R[g]=t.ids[o[1]]; TH[g]=t.ids[o[2]];
      tp[W[g]].w++; tp[R[g]].r++;
    ...
    const thirdByGroup={}; adv.forEach(x=>thirdByGroup[x.g]=x.id);
